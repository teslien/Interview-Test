  ...
  const handleAutoGenerateTest = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        localStorage.setItem('gemini_api_key', autoGenerateData.geminiApiKey);
        
        toast.success(`Test "${response.data.test.title}" generated successfully!`);
        setShowAutoGenerateDialog(false);
        setAutoGenerateData({ topic: '', questionCount: 10, geminiApiKey: autoGenerateData.geminiApiKey, level: 'intermediate' });
              ...
              <div>
                ...
                <div className="flex space-x-2 mt-1">
                  <Input
                    ...
                    disabled={autoGenerateLoading}
                  />
                  {autoGenerateData.geminiApiKey && (
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
                      onClick={() => {
                        setAutoGenerateData({ ...autoGenerateData, geminiApiKey: '' });
