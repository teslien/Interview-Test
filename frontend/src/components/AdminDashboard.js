...
import { Plus, Send, Eye, Users, FileText, Clock, Video, LogOut, Trash2, Edit, Bell, BellRing, CheckSquare, AlertCircle, Settings, Mail, UserCheck, UserX, Shield, MoreVertical, AlertTriangle, RefreshCw, CheckCircle } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const [inviteStep, setInviteStep] = useState(1); // 1: Basic Info, 2: Scheduling, 3: Review
  ...
  const handleGeminiAutoReview = async (submissionId) => {
    ...
    try {
      const response = await axios.post(`${API}/admin/auto-review/${submissionId}`, {
      ...
      if (response.data.success) {
        ...
        toast.error('Auto-review failed: ' + response.data.message);
      }
    } catch (error) {
      console.error('Auto-review error:', error);
      let message = 'Failed to perform auto-review';
  ...
  const handleOpenSendInvite = (testId = null) => {
    if (tests.length === 0) {
      toast.error('Please create a test first before sending invites');
      return;
    }
    
    // Always open the wizard, with or without pre-selected test
    setNewInvite({ 
      test_id: testId || (tests.length === 1 ? tests[0].id : ''), 
      applicant_email: '', 
      applicant_name: '', 
      scheduled_date: null, 
      scheduled_time: '', 
  ...
  const handleNextStep = () => {
    if (inviteStep === 1) {
      // Validate basic info including test selection
      if (!newInvite.test_id) {
        toast.error('Please select a test');
        return;
      }
      if (!newInvite.applicant_name || !newInvite.applicant_email) {
        toast.error('Please fill in applicant name and email');
        return;
      }
      setInviteStep(2);
    } else if (inviteStep === 2) {
      // Validate scheduling if admin scheduled
      if (newInvite.admin_scheduled && (!newInvite.scheduled_date || !newInvite.scheduled_time)) {
        toast.error('Please select date and time for scheduled test');
        return;
      }
      setInviteStep(3);
    }
  ...
  const handleSendInvite = async () => {
    ...
    if (newInvite.admin_scheduled && (!newInvite.scheduled_date || !newInvite.scheduled_time)) {
      toast.error('Please select both date and time for scheduled test');
      return;
    }
    ...
    try {
      // Prepare invite data
      const inviteData = {
        test_id: newInvite.test_id,
        applicant_email: newInvite.applicant_email,
        applicant_name: newInvite.applicant_name,
        admin_scheduled: newInvite.admin_scheduled,
        no_schedule: newInvite.no_schedule
      };
      
      // Add scheduled date if admin scheduled
      if (newInvite.admin_scheduled && newInvite.scheduled_date && newInvite.scheduled_time) {
        // Combine date and time
        const [time, period] = newInvite.scheduled_time.split(' ');
        const [hours, minutes] = time.split(':');
        let hour24 = parseInt(hours);
        if (period === 'PM' && hour24 !== 12) hour24 += 12;
        if (period === 'AM' && hour24 === 12) hour24 = 0;
        
        const scheduledDateTime = new Date(newInvite.scheduled_date);
        scheduledDateTime.setHours(hour24, parseInt(minutes), 0, 0);
        
        inviteData.scheduled_date = scheduledDateTime.toISOString();
      }
      ...
      if (invite.email_sent) {
        ...
        if (newInvite.no_schedule) {
          message = 'Invite sent successfully! Applicant can take the test anytime.';
        } else if (newInvite.admin_scheduled) {
          message = `Invite sent successfully! Test scheduled for ${newInvite.scheduled_date.toLocaleDateString()} at ${newInvite.scheduled_time}`;
        }
        toast.success(message);
      } else {
        toast.warning('Invite created but email failed to send. You can contact the applicant directly or retry sending the email.');
      }
  ...
  const handleRetryEmail = async (inviteId) => {
    try {
      const response = await axios.post(`${API}/invites/${inviteId}/retry-email`);
      ...
      fetchData(); // Refresh the invites list
    } catch (error) {
      console.error('Failed to retry email:', error);
      toast.error('Failed to retry email: ' + (error.response?.data?.detail || error.message));
    }
  };
  ...
  const handleAutoGenerateTest = async () => {
    ...
    try {
      const response = await axios.post(`${API}/tests/auto-generate`, {
        ...
        questionType: autoGenerateData.questionType
      });

      if (response.data.success) {
        toast.success(`Test "${response.data.test.title}" generated successfully!`);
        setShowAutoGenerateDialog(false);
        setAutoGenerateData({ topic: '', questionCount: 10, level: 'intermediate', questionType: 'multiple_choice' });
  ...
  return (
    <div className={`min-h-screen themed-component ${currentTheme !== 'light' ? `theme-${currentTheme}` : ''}`} style={{
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        {invite.email_sent_at && (
                          <p className="text-xs text-gray-500 mt-1">
                            Email sent: {new Date(invite.email_sent_at).toLocaleDateString()}
                          </p>
                        )}
                      </div>
                    </div>
                        ...
                        <CardContent className="p-6">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex flex-col space-y-2">
                              ...
                              <Button
                                onClick={() => handleGeminiAutoReview(submission.submission_id)}
          ...
          <div className="bg-white rounded-lg max-w-md w-full">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <Select 
                  value={autoGenerateData.questionType} 
                  onValueChange={(value) => setAutoGenerateData({ ...autoGenerateData, questionType: value })}
                  disabled={autoGenerateLoading}
                >
                  ...
                  <SelectContent>
                    <SelectItem value="multiple_choice">Multiple Choice Questions (MCQ)</SelectItem>
                    <SelectItem value="coding">Coding Questions</SelectItem>
                    <SelectItem value="mixed">Mixed (MCQ + Coding)</SelectItem>
                  </SelectContent>
                </Select>
                <p className="text-sm text-gray-500 mt-1">Choose the type of questions to generate</p>
              </div>
      ...
      {/* Send Invite Modal - Step by Step Wizard */}
      {showSendInvite && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-[100]">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-blue-100 mt-1">Step {inviteStep} of 3</p>
                </div>
                <button
                    ...
                    <div key={step} className="flex items-center">
                      ...
                      {step < 3 && (
                        <div className={`w-12 h-1 mx-2 ${
                          step < inviteStep ? 'bg-white' : 'bg-blue-500'
                        }`} />
                      )}
                    </div>
                  ))}
              ...
              {inviteStep === 1 && (
                <div className="space-y-6">
                  <div className="text-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">Test & Applicant Information</h3>
                    <p className="text-gray-600">Select a test and enter the applicant's details</p>
                  </div>

                  <div className="space-y-4">
                    <div>
                      <Label htmlFor="test-selection" className="text-sm font-medium text-gray-700 mb-2 block">
                        Select Test *
                      </Label>
                      <Select
                        ...
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            <p className="font-medium text-gray-900">
                              {tests.find(test => test.id === newInvite.test_id)?.title}
                            </p>
                            <p className="text-sm text-gray-600 mt-1">
                              {tests.find(test => test.id === newInvite.test_id)?.description}
                            </p>
                            <div className="flex items-center space-x-4 mt-2 text-xs text-gray-500">
                              <span>{tests.find(test => test.id === newInvite.test_id)?.questions.length} questions</span>
                              <span>{tests.find(test => test.id === newInvite.test_id)?.duration_minutes} minutes</span>
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              {inviteStep === 2 && (
                <div className="space-y-6">
                  <div className="text-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">Scheduling Options</h3>
                    <p className="text-gray-600">Choose how the test should be scheduled</p>
                  </div>

                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div
                        onClick={() => setNewInvite({ ...newInvite, admin_scheduled: false, scheduled_date: null, scheduled_time: '', no_schedule: false })}
                        className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                          !newInvite.admin_scheduled && !newInvite.no_schedule
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <input
                            type="radio"
                            checked={!newInvite.admin_scheduled && !newInvite.no_schedule}
                            onChange={() => {}}
                            className="h-4 w-4 text-blue-600"
                          />
                          <div>
                            <h4 className="font-medium text-gray-900">Self-Schedule</h4>
                            <p className="text-sm text-gray-600">Let applicant choose their own time</p>
                          </div>
                        </div>
                      </div>

                      <div
                        onClick={() => setNewInvite({ ...newInvite, admin_scheduled: true, no_schedule: false })}
                        className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                          newInvite.admin_scheduled
                            ? 'border-blue-500 bg-blue-50'
                            : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        <div className="flex items-center space-x-3">
                          <input
                            type="radio"
                            checked={newInvite.admin_scheduled}
                            onChange={() => {}}
                            className="h-4 w-4 text-blue-600"
                          />
                          <div>
                            <h4 className="font-medium text-gray-900">Admin-Schedule</h4>
                            <p className="text-sm text-gray-600">You choose the date and time</p>
                          </div>
                        </div>
                      </div>

                      <div
                        onClick={() => setNewInvite({ ...newInvite, admin_scheduled: false, scheduled_date: null, scheduled_time: '', no_schedule: true })}
                        className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                          newInvite.no_schedule
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <input
                            type="radio"
                            checked={newInvite.no_schedule}
                            onChange={() => {}}
                            className="h-4 w-4 text-green-600"
                          />
                          <div>
                            <h4 className="font-medium text-gray-900">No Schedule</h4>
                            <p className="text-sm text-gray-600">Applicant can take test anytime</p>
                          </div>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="bg-blue-50 rounded-lg p-6 space-y-6">
                        <div>
                          ...
                          <div className="flex justify-center">
                            <DatePicker
                              selected={newInvite.scheduled_date}
                              onChange={(date) => setNewInvite({ ...newInvite, scheduled_date: date })}
                              minDate={new Date()}
                              ...
                              dayClassName={(date) => {
                                ...
                                const isSelected = newInvite.scheduled_date && date.toDateString() === newInvite.scheduled_date.toDateString();
                        ...
                        {newInvite.scheduled_date && newInvite.scheduled_time && (
                          <div className="bg-white p-4 rounded-lg border border-blue-200">
                            <div className="flex items-center space-x-3">
                              <CheckCircle className="h-5 w-5 text-green-600" />
                              <div>
                                <p className="font-medium text-gray-900">Test Scheduled</p>
                                <p className="text-sm text-gray-600">
                                  {newInvite.scheduled_date.toLocaleDateString('en-US', { 
                                    weekday: 'long', 
                                    year: 'numeric', 
                                    month: 'long', 
                                    day: 'numeric' 
                                  })} at {newInvite.scheduled_time}
                                </p>
                              </div>
                            </div>
                          </div>
              ...
              {inviteStep === 3 && (
                <div className="space-y-6">
                  <div className="text-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">Review & Send</h3>
                    <p className="text-gray-600">Review the details before sending the invitation</p>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 space-y-4">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{newInvite.applicant_name}</p>
                        <p className="text-sm text-gray-500">{newInvite.applicant_email}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {newInvite.no_schedule 
                            ? 'No schedule - applicant can take test anytime'
                            : newInvite.admin_scheduled 
                              ? `Scheduled for ${newInvite.scheduled_date?.toLocaleDateString()} at ${newInvite.scheduled_time}`
                              : 'Applicant will choose their own time'
                          }
                        </p>
                      </div>
                    </div>
            ...
            {/* Footer */}
            <div className="bg-gray-50 px-6 py-4 flex justify-between border-t border-gray-200 flex-shrink-0">
              <Button
                onClick={inviteStep === 1 ? handleCloseInvite : handlePrevStep}
                variant="outline"
                className="px-6"
              >
            ...
            <div className="p-6">
              <Tabs defaultValue="applicants" className="w-full">
                ...
                <TabsContent value="email" className="space-y-6 mt-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg font-medium">AI Test Generation</h3>
                      <Button
                        onClick={() => {
                          const stored = localStorage.getItem('gemini_api_key');
                          if (stored) {
                            setGeminiApiKey(stored);
                            toast.success('API key loaded from storage');
                          } else {
                            toast.info('No API key found in storage');
                          }
                        }}
                        variant="outline"
                        size="sm"
                    ...
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                      <div className="flex items-start space-x-2">
                        ...
                        <div>
                          <h4 className="font-medium text-green-900">AI Test Generation</h4>
                          <ul className="text-sm text-green-700 mt-1 space-y-1">
                            <li>• Store your Gemini API key securely</li>
                            <li>• Use the "Auto Generate Test" feature to create tests with AI</li>
                            <li>• API key is stored locally in your browser</li>
                            <li>• Clear the key if you want to remove it</li>
                          </ul>
                        </div>
                      </div>
                    </div>
